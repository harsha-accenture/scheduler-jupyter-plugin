...
test.describe('Vertex scheduling jobs', () => {
  test.skip('Can create job scheduler', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      await page.evaluate((filePath) => {
        const inputField = document.querySelector('input[value="Untitled.ipynb"]') as HTMLInputElement;
        if (inputField) {
          // Override the 'files' property with a DataTransfer object
          const dataTransfer = new DataTransfer();
          const file = new File(['dummy content'], filePath.split('/').pop() || 'file.ipynb');
  ...
  test('Create new bucket', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      await expect(page.getByText(`Create and Select "${bucketName}"`)).toBeVisible();
      
      // Create and select new bucket from dropdown
      await page.getByText(`Create and Select "${bucketName}"`).click();
      await expect(page.getByText('Select an existing bucket or create a new one.')).toBeVisible();
      ...
      await page.getByRole('option', { name: bucketName }).waitFor({ state: "attached" });
      await expect(page.getByRole('option', { name: bucketName })).toBeVisible();
      await page.getByRole('option', { name: bucketName }).click();
      await expect(page.getByLabel('Cloud Storage Bucket*')).toHaveValue(bucketName);
    }
  });
  ...
  */
  ...
  test('Sanity: can perform field validation', async ({ page }) => {
    ...
    if (kernelCount > 0) {
      ...
      await expect(page.getByText('Accelerator count is required')).toBeVisible();
      await page.getByLabel('Accelerator count*').click();
      await page.getByRole('option').first().click();
      await expect(page.getByText('Accelerator count is required')).toBeHidden();
      ...
      const ServiceAccountName = await page.getByLabel('Service account*').innerText();
      await expect(page.getByLabel('Service account*')).toHaveValue('Compute Engine default service account');
...
test.describe('Vertex scheduling jobs listing page validation', () => {
  ...
  test('Refresh listing screen', async ({ page }) => {
    ...
    await page.getByText('Loading Vertex Schedules').waitFor({ state: "hidden" });
  ...
  test('Sanity: Delete a job', async ({ page }) => {
    ...
    if (jobLocator) {
      ...
      await page.getByText('Loading Vertex Schedules').waitFor({ state: "hidden" });
      await page.locator('//table[@class="clusters-list-table"]').isVisible();
      await expect(jobLocator.locator('//td[@role="cell"][1]')).not.toEqual(jobName);
